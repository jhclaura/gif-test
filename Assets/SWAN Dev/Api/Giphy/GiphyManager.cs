...
public class GiphyManager : MonoBehaviour
{
	...
	private bool HasUserName
	{
		get{
			...
			if(!hasUserName) Debug.LogWarning("Giphy User Name is required!");
			return hasUserName;
		}
	}
	...
	public void Upload(string filePath, Action<GiphyUpload.Response> onComplete, Action<float> onProgress = null)
	{
		if(!HasUploadApiKey || !HasUserName) return;
		StartCoroutine(_Upload(filePath, null, onComplete, onProgress));
	}
	...
	/// <param name="onComplete">On complete.</param>
	/// <param name="onProgress">On progress.</param>
	public void Upload(string filePath, List<string> tags, Action<GiphyUpload.Response> onComplete, Action<float> onProgress = null)
	{
		if(!HasUploadApiKey || !HasUserName) return;
		StartCoroutine(_Upload(filePath, tags, onComplete, onProgress));
	}

	IEnumerator _Upload(string filePath, List<string> tags, Action<GiphyUpload.Response> onComplete, Action<float> onProgress = null)
	{
