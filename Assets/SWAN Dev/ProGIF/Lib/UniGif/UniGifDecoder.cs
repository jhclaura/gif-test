...
{
	public static List<GifTexture> gifTexList = null;

	//Decode settings
	private static ProGifPlayerComponent.DecodeMode decodeMode = ProGifPlayerComponent.DecodeMode.Normal;
	private static int targetDecodeFrameNum = -1; //if targetDecodeFrameNum <= 0: decode & play all frames (+/- 1 frame)
	private static float previewFrameSkipNum = 0f;

	public static void SetDecodeSettings(ProGifPlayerComponent.DecodeMode inDecodeMode, int inMaxDecodeFrameCount)
	{
		...
		FilterMode filterMode, TextureWrapMode wrapMode, Action<GifTexture> onFrameReady = null)
    {
        ...
        for (int i = 0; i < gifData.m_imageBlockList.Count; i++)
        {
			...
			if(doDecode)
			{
				...
				//Preview mode decode
				if(decodeMode == ProGifPlayerComponent.DecodeMode.Advanced && previewFrameSkipNum > 0) delaySec *= previewFrameSkipNum;

	            // Add to GIF texture list
				GifTexture gTex = new GifTexture(tex, delaySec);
				gifTexList.Add(gTex);
				if(onFrameReady != null) onFrameReady(gTex);
